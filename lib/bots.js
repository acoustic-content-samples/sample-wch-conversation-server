...
conversationmiddleware.before = function(message, conversationPayload, callback) {
  ...
  let clienttypePromise = new Promise((resolve, reject) => {
    try {
      let clienttype;
      switch (message.type) {
          case 'message.rest':
  ...
  let languagePromise = new Promise((resolve, reject) => {
    if (languagetranslator && _conversationPayload.context && !_conversationPayload.context.outputlang) {
      languagetranslator.identify(
        ...
        (err, language) => {
          if (err) {
            ...
            debug('Identified language %o', language.languages[0]);
      ...
      debug('Language already set to %s', _conversationPayload.context.outputlang);
      resolve(_conversationPayload.context.outputlang);
    }
  });
  ...
  let geolocationPromise = new Promise((resolve, reject) => {
    if (geolocation && (_conversationPayload.context && _conversationPayload.context.setlocation === true)) {
      debug('Looking for geolocation %s', message.text);
      geolocation.geocode({
  ...
  Promise.all([clienttypePromise, languagePromise, tonePromise, geolocationPromise])
