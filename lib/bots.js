...
const botCreds = appEnv.credentials.getService('bot_config');
...
const { bot_config = {}, geolocationservice = {}, wch_toneanalyzer = {}  } = appEnv.settings; // eslint-disable-line camelcase
...
const supportedLanguages = bot_config.supportedLanguages; // eslint-disable-line camelcase
const defaultLanguage = bot_config.defaultLanguage; // eslint-disable-line camelcase
...
const identifyLanguage = function (msgText, locale, context) {
  return new Promise((resolve, reject) => {
    if(locale && context && context.setoutputlang) {
      debug('Specific locale set for message: %s', locale);
      resolve(locale.split('-')[0]);
    }
    else if (languagetranslator &&  context && context.setoutputlang) {
      languagetranslator.identify(
        ...
        (err, language) => {
          if (err) {
            ...
            && supportedLanguages.includes(language.languages[0].language)) {
      ...
      debug('No context yet! Set default: %s', defaultLanguage);
      resolve(defaultLanguage);
    }
  });
}

const identifyTone = function (msgText, context) {
  return new Promise((resolve, reject) => {
    if (wch_toneanalyzer.enabled && toneanalyzer) { // eslint-disable-line camelcase
      toneanalyzer.tone({
        text: msgText,
        language: (context && context.outputlang) ? context.outputlang : defaultLanguage
      },
      (err, tone) => {
        if (err) {
...
const identifyGeolocation = function (msgText, context) {
  return new Promise((resolve, reject) => {
    if (geolocationservice.enabled && geolocation && (context && context.setlocation === true)) {
...
const initBots = function (app) {
  // Init Slack
  if (bot_config.enabled.slack) { // eslint-disable-line camelcase
    ...
    if (slackApp) {
      ...
      Slack.controller.createHomepageEndpoint(slackRouter);
      Slack.controller.createOauthEndpoints(slackRouter);
      Slack.controller.createWebhookEndpoints(slackRouter, [botCreds.credentials.verificationtoken]);
      app.use('/slackendpoint', slackRouter);
      debug('Slack app is live...');
    }
    if (botCreds && botCreds.credentials && botCreds.credentials.testbot) {
      ...
      Slack.controller.spawn({
        token: botCreds.credentials.testbot
      }).startRTM();
      debug('Testbot is live...');
    }
  }
  // Init facebook
  if (bot_config.enabled.facebook) { // eslint-disable-line camelcase
