...
const { bot_config = {}, geolocationservice = {}, wch_toneanalyzer = {}  } = appEnv.settings;
const modeDev = bot_config.enabled.developermode;
const slackApp = bot_config.enabled.isslackapp;
const supportedLanguages = bot_config.supportedLanguages;
const defaultLanguage = bot_config.defaultLanguage;
...
const identifyTone = function (msgText, context) {
  return new Promise((resolve, reject) => {
    if (wch_toneanalyzer.enabled && toneanalyzer) {
      debug(msgText)
      debug(context.outputlang)
      toneanalyzer.tone({
        text:msgText,
...
const initBots = function (app) {
  ...
  // Init facebook
  if (bot_config.enabled.facebook) {
  ...
  if (bot_config.enabled.raspberry) {
  ...
  if (bot_config.enabled.alexa) {
    let Alexa = require('./bot-alexa')(conversationmiddleware);
