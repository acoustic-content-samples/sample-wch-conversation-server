...
const debug = require('debug')('wchbotserver:restmessagehandling');
...
module.exports = (controller, {wchcore, appSettings}) => {
  ...
  controller.hears('.*', ['message_received'], function(bot, message) {
    ...
    let userLocale = (message.locale) ? message.locale.split('-')[0] : generalSettings.defaultLanguage;
      ...
      if (err) {
        ...
        wchconversation.getWchConversationResponses(watsonData)
          ...
          .then(responses => {
            ...
            return transformToOutputMessage({
...
const transformToOutputMessage = function (resultSet, modeDev) {
    ...
    if (searchResult.numFound > 0) {
      ...
      if (followup) {
        Object.assign(details, {
          "FollowupAction": watsonData.output.action,
          "ContentType": "ChatFollowup"
        });
      }
      else {
        Object.assign(details, {
          "DialogNode": watsonData.output.nodes_visited[watsonData.output.nodes_visited.length-1],
          "NodeResponse": watsonData.context.nodetype && watsonData.context.nodetype === 'response_condition' ? watsonData.context.nodename : 'None',
          "Entities": watsonData.entities && watsonData.entities.length > 0 ? watsonData.entities.map(ele => ele.value).join(' , ') : 'None',
          "ContentType": 'ChatOutputText',
        });
      }
      // General Information
      Object.assign(details, {
        "OutputLanguage": watsonData.context.outputlang,
        "ConversationOutput": watsonData.output.text && watsonData.output.text.length > 0 ? watsonData.output.text[watsonData.output.text.length-1] : 'None'
      });
