...
const transformToSlackMessage = function (resultSet) {
  return new Promise((resolve, reject) => {
    ...
    if (searchResult.numFound > 0) {
      ...
      //   ${watsonData.output.nodes_visited[watsonData.output.nodes_visited.length-1]}
      //   \n *Recognized Entities:*
      //   ${watsonData.entities && watsonData.entities.length > 0 ? watsonData.entities.map(ele => ele.value).split() : 'None'} \n
      //   *Nodename:* ${watsonData.context.nodetype && watsonData.context.nodetype === 'response_condition' ? watsonData.context.nodename : 'None'} \n *OutputLang:* ${watsonData.context.outputlang} \n *Action:* ${JSON.stringify(watsonData.output, null, 3)}`});
      resolve(
        {
