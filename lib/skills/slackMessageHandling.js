...
module.exports = (controller, {wchcore, appSettings}) => {
  ...
  const avatars = channels.slack.chatboticons;
          ...
          if (err) {
            ...
            wchconversation.getWchConversationResponses(watsonData)
              ...
              .then(({respToUse, followupResp}) => {
                ...
                return transformToSlackMessage({watsonData, searchResult, attachmentsResult, wchhost, avatars, modeDev, followup: false})
              ...
              .then(followupResp => {
                ...
                transformToSlackMessage({watsonData, searchResult, attachmentsResult, wchhost, avatars, modeDev, followup: true})
...
const transformToSlackMessage = function (resultSet) {
  return new Promise((resolve, reject) => { // eslint-disable-line complexity
    let {watsonData, searchResult, attachmentsResult = {}, wchhost, avatars, modeDev, followup} = resultSet;
    let attachments = [];
      ...
      try {
        attachments = attachmentsResult.documents.reduce((arr, attachment) => {
          let slackattachment = Object.keys(attachment.document.elements)
            .reduce((attach, key) => { // eslint-disable-line complexity
              let value = undefined;
    ...
    if (searchResult.numFound > 0) {
      ...
      if (persona && persona !== '' && avatars[persona] !== null && avatars[persona] !== "") {
        output.username = persona;
        output.icon_url = avatars[persona]; // eslint-disable-line camelcase
      }
      resolve(output);
    }
    else if (modeDev) {
      ...
      if (followup) {
        detailsFields.push({
          ...
          "value": `${watsonData.output.action}`,
          "short": true
        });
        ...
        detailsFields.push({
          "title": "Dialog Node (Node)",
          "value": `${watsonData.output.nodes_visited[watsonData.output.nodes_visited.length-1]}`,
          "short": true
        });

        detailsFields.push({
          "title": "Node Response (Nodename)",
          "value": `${watsonData.context.nodetype && watsonData.context.nodetype === 'response_condition' ? watsonData.context.nodename : 'None'}`,
          "short": true
        });

        detailsFields.push({
          "title": "Recognized Entities (Filter)",
          "value": `${watsonData.entities && watsonData.entities.length > 0 ? watsonData.entities.map(ele => ele.value).join(' , ') : 'None'}`,
          "short": true
        });
      ...
      detailsFields.push({
        ...
        "value": `${watsonData.context.outputlang}`,
        "short": true
      });

      resolve(
        {
          "text": "There is no answer for this question yet!",
          "attachments": [
            {
              "Title": "Details",
            ...
            },
            {
              ...
              "fields": [
                {
                  "title": "Debug",
                  "value": `${watsonData.output.text && watsonData.output.text.length > 0 ? watsonData.output.text.join('\n') : "None"}`
                }
              ]
            },
            // TODO Make this more convenient and automatically create the draft content
            // {
            //   "title": "Editor Actions",
            //   "callback_id": "create_response",
            //   "actions": [
            //     {
            ...
            //       "type": "button",
            //       "style": "primary",
            //       "value": JSON.stringify(detailsFields)
            //     }]
            // }
          ]
        }
      ...
      resolve({text: `${watsonData.output.text && watsonData.output.text.length > 0 ? watsonData.output.text[watsonData.output.text.length-1] : "That's a good question. Sadly I don't know the answer (yet) :cry:"}`});
    }
  });
}
