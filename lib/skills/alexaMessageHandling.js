...
const debug = require('debug')('wchbotserver:alexamessagehandling');
...
module.exports = (controller, {wchcore, appSettings}) => {
  ...
  controller.hears('.*', ['message_received'], function(bot, message) {
    ...
    let userLocale = message.locale.split('-')[0];
    ...
    conversationMiddleware.get(locale).interpret(bot, message, function (err) {
      if (err) {
        errHandler({err, bot, message, reason: 'Conversation Service does not respond.'})
      }
      else {
