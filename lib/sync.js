...
/**
 * Synchronization tool between Watson Conversation Service and Watson Content Hub.
 * The tool will push all intents, entities (and their values but no synonyms), dialog nodes
 * and actions (custom behaviour to trigger specific actions) to WCH as a taxonomy. Cúrrently
 * this process is one way from WCS to WCH.
 */
/* eslint camelcase: 0 */
'use strict';
...
const getWorkspace = function () {
  ...
  return new Promise((res, rej) => {
    ...
    (err, resp) => {
      ...
      // debug('Retrieved: %s', JSON.stringify(resp, null, 1));
      res(resp);
    });
  });
}
