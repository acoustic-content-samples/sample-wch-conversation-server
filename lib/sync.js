...
function transformToTaxonomy(workspace) {
  return new Promise((res, rej) => {
    try {
      ...
      console.log(dialog_nodes);

      dialog_nodes.forEach(dialogNode => {
        let {context : { wchid, nodename}, parent, dialog_node} = dialogNode;
        let childname = (nodename) ? nodename : dialog_node;

        if(!parent || parent === null || parent === 'null') {
          parent = 'dialog_nodes';
        }
        console.log('wchid ', wchid);
        if(wchid){
      ...
      console.log('taxonomies.dialog_nodes ', JSON.stringify(taxonomies.dialog_nodes, null, 1));
