...
const AlexaBot = function (configuration) {
  ...
  alexaBotkit.middleware.spawn.use((bot, next) => {
    ...
    bot.router.post('/message', (req, resp) => {
      ...
      if (session.user.hasOwnProperty('userId') && session.user.userId.length > 100){
        // strip too long userId. First part of userId is not unique
        session.user.userId = session.user.userId.substr(19, 100);
      }
      switch (request.type) {
          case 'LaunchRequest':
            ...
            alexaBotkit.ingest(bot, launchmessage, resp);
            break;
          case 'IntentRequest':
            ...
            alexaBotkit.ingest(bot, intentmessage, resp);
            break;
          case 'SessionEndedRequest':
            ...
            alexaBotkit.ingest(bot, sessionEnd, resp);
            break;
          default:
  ...
  alexaBotkit.middleware.ingest.use(function(bot, message, resp, next) {
    message.resp = resp;
    next();
  });
