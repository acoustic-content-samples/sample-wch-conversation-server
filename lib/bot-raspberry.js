...
const transformToAudioMessage = function (resultSet) {
  return new Promise((resolve, reject) => {
    ...
    if (attachmentsResult.numFound && attachmentsResult.numFound > 0) {
      try {
        attachments = attachmentsResult.documents.reduce((str, attachment) => {
          let slackattachment = attachment.document.elements[ELE_FALLBACK].value;
          if (attachment.document.elements.fallback && attachment.document.elements[ELE_FALLBACK].value) {
            return str.concat(" "+attachment.document.elements[ELE_FALLBACK].value);
          }
          else {
            return str;
          }
    ...
    if (searchResult.numFound > 0) {
      ...
      resolve({text: `${doc.elements.text.value || sample(doc.elements.text.values)} ${attachments} ${followup}`});
    }
    else {
...
module.exports = function (middleware) {
  controller.hears('.*', ['message_received'], function(bot, message) {
    if (message.match[1] === '') {
      return bot.reply(message, "I can't help you unless you type in anything!");
    }
