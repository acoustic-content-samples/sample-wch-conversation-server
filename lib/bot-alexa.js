...
const AlexaMessageHandling = require('./skills/alexaMessageHandling');
...
module.exports = function ({app, wchcore, appSettings}) {
  ...
  return new Promise((resolve, reject) => {
    if (alexaSettings.enabled) {
      ...
      else if (alexaSettings.fileStore !== false) {
        if (dbService.enabled && !isDbCredsSet) {
      ...
      AlexaMessageHandling(controller, {wchcore, appSettings});
