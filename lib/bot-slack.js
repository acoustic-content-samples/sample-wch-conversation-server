...
const mode_dev = appEnv.settings.bot_config.enabled.developermode;
...
const transformToSlackMessage = function (resultSet) {
  return new Promise((resolve, reject) => {
    ...
    if(searchResult.numFound > 0) {
      ...
      resolve({text: `${doc.elements.text.value} ${icon}`, attachments: attachments, icon_url: icons[watsonData.context.chatbotpersona]});
    } else {
...
module.exports = function(middleware) {
  ...
  controller.hears(['^(?!\\.\\.\\.)(.*)'], ['direct_message', 'direct_mention', 'mention'], function(bot, message) {
    ...
    bot.reply(message, {text:`:hammer: Sorry I'm currently under development and only online for specific test purposes. Try again later :hammer:`});
  });
