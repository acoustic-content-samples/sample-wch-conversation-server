...
if (dbConfig.credentials && dbConfig.credentials.mongoUri) {
    const mongoStorage = require('botkit-storage-mongo')({mongoUri: dbConfig.credentials.mongoUri});
...
const app_options = {
    ...
    redirectUri: 'https://wch-conv-int-srv.mybluemix.net/oauth',
    scopes: ['bot', 'commands', 'chat:write:bot', 'emoji:read', 'im:read', 'channels:history']
};

const controller = Botkit.slackbot(bot_options).configureSlackApp(app_options);
require('./skills/interactivemessages')(controller);

const botContent = controller.spawn({
  token: botConfig.credentials.profcontent
});
const botWatson = controller.spawn({
  token: botConfig.credentials.mrwatson
});
...
const transformToSlackMessage = function (resultSet) {
  return new Promise((resolve, reject) => {
      ...
      try {
        attachments = attachmentsResult.documents.reduce((arr, attachment) => {
            ...
            reduce((attach, key) => {
              ...
              switch (key) {
                case 'callback_id':
                  value = {[key] : currEle.value, attachment_type:'default'};
                  break;
                case 'text':
                case 'color':
                case 'fallback':
    ...
    if(searchResult.numFound > 0) {
      ...
      resolve({text: `${doc.elements.text.value} ${icon}`, attachments: attachments, icon_url:'https://my6.digitalexperience.ibm.com/8f032d79-a358-4825-af1b-d6cc169692c6/dxdam/df/df0b0e06-40c4-45a1-8d93-4b0fe45679fe/DrHub.PNG'});
    } else {
