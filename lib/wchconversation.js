  ...
  return new Promise((resolve, reject) => {
    ...
    if (numFound > 1) {
      ...
      if(filtered.length === numFound) {
        ...
        // Filter for content item with the smallest amount of categories...
         let minCount = 9999999;
         filtered = filtered.map(ele => {
            if(ele.document.elements.filter && ele.document.elements.filter.categories) {

              if(ele.document.elements.filter.categories.length < minCount) {
                minCount = ele.document.elements.filter.categories.length;
              }
              return {origin:ele, transf: ele.document.elements.filter.categories.length};
            } else {
              return {origin:ele, transf: 0};
            }
          })
